  ...
  protected async generateAutoPoster(
    ...
    try {
      ...
      const finalCollectionType =
        options?.collectionTypeOverride || config.type;
      ...
      if (resolvedTemplateId) {
        try {
          ...
          logger.warn(
            'Failed to load poster template data for change detection',
            {
              label: `${this.source} Collections`,
              configId: config.id,
      ...
      if (items && items.length > 0) {
        ...
        const templateElements =
          (templateData as {
            elements?: { type?: string; properties?: unknown }[];
          })?.elements;

        if (templateElements && templateElements.length > 0) {
          const contentGridElements = templateElements.filter(
            (element) => element?.type === 'content-grid'
          );

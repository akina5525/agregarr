...
export async function completeAutoGeneratedPosterWorkflow(
  ...
  try {
    const axios = await import('axios');
    const tempPath = path.join(POSTER_STORAGE_DIR, tempFilename);
    let plexBuffer: Buffer;

    const protocol = (() => {
      try {
        return new URL(plexPosterUrl).protocol;
      } catch {
        return null;
      }
    })();
    ...
    if (protocol && ['http:', 'https:'].includes(protocol)) {
      ...
      plexBuffer = Buffer.from(response.data);
    } else {
      logger.info(
        'Plex returned an upload:// URL, skipping re-download and using the generated file hash instead',
        {
          collectionIdentifier,
          plexPosterUrl,
        }
      );
      plexBuffer = await fs.promises.readFile(tempPath);
    }
